...
public class Heap<E> {
    ...
    protected void ensureSpace() {
    	...
    	if (size == d.length) {
    		VP[] doubled = Arrays.copyOf(this.d, size*2 );
    		d = doubled;
    	}
    }
    ...
    protected int upperChild(int n) {
        ...
        else if (2*n + 1 == size-1) {
        	return 2*n + 1;
        }
        else if (d[2*n+1].priority == d[2*n+2].priority) {
        	return 2*n+2;
        	}
        	else 
        return compareTo(2*n+1,2*n+2) == 1 ? 2*n+1 : 2*n+2;
    }
